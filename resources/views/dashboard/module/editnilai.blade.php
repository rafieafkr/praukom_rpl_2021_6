...
{{csrf_field()}}
      ...
      <div class=" w-full px-2  font-bold">
        ...
        <input type="hidden" name='kompetensi' value="{{$d->kompetensi}}" class="rounded-md shadow-inner w-full border border-gray-400">
        
        <br>

        <label for="">Nilai</label>
        <input type="number" max="100" name="nilai" value="{{$d->nilai}}" class="rounded-md shadow-inner w-full border border-gray-400">
       

        
        <button type="submit" value="simpan" class="btn bg-[#256D85] rounded-lg px-5 mb-5 py-3 text-center shadow-md hover:bg-emerald-700 font-bold text-white">Simpan</button>
        </div>
</div>
